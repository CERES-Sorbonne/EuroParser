...
then
    ...
    screen -S EuropressParser -dm bash -c "source $FOLDER/venv/bin/activate; python -m uvicorn europarser_api.api:app --port $EUROPARSER_PORT --root-path /europarser --workers 8 --limit-max-requests 8 --timeout-keep-alive 1000 --log-config log.conf"
else
