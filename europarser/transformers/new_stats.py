...
def int_to_monthyear(i: int) -> str:
    return to_monthyear(int_to_datetime(i))
...
class StatsTransformer:
    ...
    def transform(self, pivot_list):
        ...
        df['mois'] = df['epoch'].map(int_to_monthyear)
...
if __name__ == '__main__':
    ...
    with open(f"pivots{mode}.json", "r") as f:
        dict_ = json.load(f)
    ...
    with open(f"stats{mode}.json", "w") as f:
        json.dump(transformer.data, f, indent=4)

    with open(f"profiler{mode}.txt", "w") as f:
        f.write(s.getvalue())
